...
|------|-------|------------|--------|
| `JobId` | `String` | `batchalign-types/src/domain/` | Job identity |
| `CommandName` | `String` | `batchalign-types/src/domain/command.rs` | Batchalign command (`"morphotag"`, `"align"`) |
...
| `NumSpeakers` | `u32` | `batchalign-types/src/domain/` | Speaker count for diarization |
| `DurationSeconds` | `f64` | `batchalign-types/src/domain/` | Duration in seconds |
| `UnixTimestamp` | `f64` | `batchalign-types/src/domain/` | Epoch timestamp |
| `DurationMs` | `u64` | `batchalign-types/src/domain/` | Duration in milliseconds |
| `MemoryMb` | `u64` | `batchalign-types/src/domain/` | Memory amount in megabytes |
| `WorkerPid` | `u32` | `batchalign-types/src/worker.rs` | OS process ID |
| `AsrTimestampSecs` | `f64` | `batchalign/src/asr_postprocess/asr_types.rs` | Raw ASR provider timestamp (seconds) |
| `SpeakerIndex` | `usize` | `batchalign/src/asr_postprocess/asr_types.rs` | Zero-based speaker index in a recording |
...
`crates/batchalign-types/src/worker_v2.rs` (re-exported via
`crates/batchalign/src/types/worker.rs`) must produce JSON identical to the
Python Pydantic models. Snapshot tests in `json_compat.rs` enforce this — if a
...
|------|--------|---------|-----------|
| 2026-02 | Text provenance newtypes | Provenance | Bugs from mixing raw/cleaned text; types in `batchalign/src/text_types.rs` |
