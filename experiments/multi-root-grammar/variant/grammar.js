...
export default grammar({
  ...
  rules: {
    ...
    document: $ => choice(
      prec(5, $.full_document),    // complete @UTF8...@Begin...@End file
      prec(4, $.utterance),        // main tier + dependent tiers
      prec(3, $.main_tier),        // single *SPEAKER:\tcontent terminator
      prec(3, $.dependent_tier),   // single %tier:\tcontent
      prec(3, $.header),           // single @Header:\tcontent
      prec(3, $.pre_begin_header), // @PID, @Font, @Window, @Color words
      prec(1, $.mor_contents),     // bare mor words: v|want qn|more
      prec(1, $.gra_contents),     // bare gra relations: 1|2|SUBJ 2|0|ROOT
      prec(0, $.contents),         // bare main tier content: hello world .
    ),
